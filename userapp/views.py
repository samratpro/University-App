...
def profile(request):
    ...
    if request.method == 'POST':
        ...
        if deparment_id:
            deparment_name = Deperment.objects.get(pk=deparment_id).deperment_name
            user_profile.deperment = deparment_name
        if semester_id:
            semester_name = Semester.objects.get(pk=semester_id).semester_name
            user_profile.semester = semester_name
        if admission_year:
